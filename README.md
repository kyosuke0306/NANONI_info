...
- 通常版 / 簡易版 / スケジュール の3ページ（目次・キーワード絞り込み・印刷/PDF出力対応）
- スマートフォン / ダークモード対応
...
3つのページがあります。サイドバー下部のボタンでいつでも行き来でき、
一度パスワードを入れれば同じタブ内では再入力を求められません。

| | 通常版 `index.html` | 簡易版 `simple.html` | スケジュール `schedule.html` |
|---|---|---|---|
| 内容 | 図・表・説明をすべて掲載 | **図だけ** | 工程と**進捗の記録** |
| 章の数 | 15 | 4 | 2 |
| 図の数 | 14 | 7 | 1 |
| キーワード絞り込み | あり | なし | なし |
| ページの長さ | 約15,200px | 約3,400px | 約2,500px |

### 進捗の記録について

スケジュールページでは、9つの作業に **未着手 / 進行中 / 完了** を付けられます。
...
**本文は1つしか持っていません。** `payload.js`（暗号化された本文）は3ページ共通で、
各ページは復号したあとに必要な部分だけを残して表示しています。
そのため資料を更新するときは `src/content.html` を直すだけで、3ページすべてに反映されます。
...
| `data-page="schedule"` | スケジュールページにだけ出す | `<section>` |
